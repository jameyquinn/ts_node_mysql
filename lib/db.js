...
function create(){
  pool.getConnection(function(err, connection) {
    ...
    if(connection){
      ...
      connection.query(
        "SELECT TABLE_NAME FROM information_schema.TABLES WHERE (TABLE_SCHEMA = '" + config.get('mysql:database') + "') AND (TABLE_NAME = 'google_form_data')"
        , function(err, rows, fields) {
            ...
            if(rows && rows.length === 0){
              ...
              log.info('creating table google_form_data');
              ...
              log.info("created table " + config.get('mysql:database') + ".google_form_data");
            }
            release(connection);
          }
      );
